...
async def test_protocol_version_constant_is_exported() -> None:
    ...
    version string '0.2.0' that this wrapper targets.
    """
    from amplifier_agent_client import PROTOCOL_VERSION_REQUIRED_BY_WRAPPER

    assert PROTOCOL_VERSION_REQUIRED_BY_WRAPPER == "0.2.0", (
        f"Protocol version mismatch: got {PROTOCOL_VERSION_REQUIRED_BY_WRAPPER!r}, expected '0.2.0'"
    )
