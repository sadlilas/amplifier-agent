...
export function resolveBinaryPath(opts = {}) {
    ...
    const err = new Error("amplifier-agent binary not found. " +
...
export function buildEnv(opts) {
    ...
    for (const key of Object.keys(extra)) {
        if (BLOCKED_ENV_KEYS.has(key)) {
            throw new AaaError("env_injection_rejected", `env.extra key '${key}' is blocked for security reasons (design §4.12.1).`, { classification: "protocol", severity: "error" });
        }
    }
 ...
 */
export async function probeEngineVersion(binPath, env, timeoutMs = 5000) {
